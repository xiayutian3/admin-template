      ...
      <div class="list">
        <div class="yy-table-wrap table-traits ">
          ...
          >
            ...
            <el-table-column prop="accountName" label="广告账号名称"> </el-table-column>
            <el-table-column prop="releaseTime" label="发布时间"> </el-table-column>
            <el-table-column prop="statusUpdateTime" label="状态更新时间"> </el-table-column>
            <el-table-column prop="taskStatus" label="任务状态">
               <template #default="scope">{{ collectTaskFormatStatus(scope.row.taskStatus) }}</template>
            </el-table-column>
            <el-table-column class-name="operate" label="操作" width="214">
              <template #default="scope">
                <span @click="handleClick(scope.row)" class="btn">查看明细</span><span @click="handleRemove(scope.row)" class="btn rt-btn">删除</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
import { collectGetTaskDtate, collectTaskResetSearchFn, collectTaskRemove, collectTaskFormatStatus, collectAzStoreSelecList, collectGetPubilshUser } from '../common/action/index'
  ...
  setup () {
    ...
    const handleClick = (item: any) => {
      ...
      if (item.adType == 4) {
        ...
        //  sb广告
        router.push({ name: 'AmznSbad', params: { item: JSON.stringify(item) } })
      } else if (item.adType == 6) {
        // sd广告
        router.push({ name: 'AmznSdad', params: { item: JSON.stringify(item) } })
      } else if (item.adType == 7) {
        // 广告组合
        router.push({ name: 'AmznAdcomposition', params: { item: JSON.stringify(item) } })
      }
    }
